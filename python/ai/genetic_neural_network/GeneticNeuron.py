...
class GeneticNeuron(object):
    ...
    def __init__(self, in_count, max_number=100):
        ...
        self.ga = GA(gene_size=self.nbits, population_size=1000,
                     maximization=False, epochs=50, ephoc_generations=10)
    ...
    def run(self, data, forward_penality):
        ...
        if(out_count <= 0):
            raise GeneticNeuronConfigurationError("The number of inputs({in_count}) on neuron does not match with data.shape. It must be greater than data.shape[1]. Perhaps you passed the datasite with wrong number of features ({features}) ".format(
                features=data.shape[1], in_count=self.in_count), errors=None)

        def optimize(genes, data):
            activation_bits = genes[0:self.activations_bits]
            weigths_bits = genes[self.activations_bits:]
            activation = min([bitsToBytes(activation_bits), len(
                ActivationFunctions.activations())-1])
            # print(activation)
            weights = bitsToBytes(np.reshape(
                weigths_bits, (self.in_count, -1)))
...
def main():
    ...
    # script directory
    dataset_path = os.path.abspath(os.path.join(inspect.getfile(inspect.currentframe(
    )), os.pardir, os.pardir, os.pardir, os.pardir, 'datasets', 'iris'))
    iris_dataset = pd.read_csv(os.path.join(dataset_path, 'iris.csv'))
    iris_dataset = pd.concat([iris_dataset, pd.get_dummies(
        iris_dataset['species'], prefix='class')], axis=1)
